 ...
 * @author Deril
 */
public class Render extends Mediator {
	...
	private var text_types:Dictionary = new Dictionary(); /** of TextFormat by int */
	...
	public function Render(appStage:Stage, dataStore:Storadge){
		super(NAME);
		...
		//
		this.appStage.addEventListener(Event.RESIZE, handleResize);
		...
		appStage.addChild(tabPane);
		//
		// create TextPane
		textPane = new TextPane();
		appStage.addChild(textPane);
		...
		//
		new ToolTip(this.appStage);
		//
		//show app borders
		appStage.addChild(borders);
	}
	...
	override public function listNotificationInterests():Array {
		return [ // 
			Note.RESIZE_APP, //
			Note.SCROLL_LINE_UP, //
			Note.SCROLL_FAST_SCROLL_UP, //
			Note.SCROLL_THIRD_UP, //
			Note.SCROLL_PAGE_UP, //
			Note.SCROLL_LINE_DOWN, //
			Note.SCROLL_FAST_SCROLL_DOWN, //
			Note.SCROLL_THIRD_DOWN, //
			Note.SCROLL_PAGE_DOWN, //
			];
	}

	override public function handleNotification(note:INotification):void {
		switch (note.getName()){
			case Note.RESIZE_APP:
				handleResize();
				break;
			case Note.SCROLL_LINE_UP:
				scrollTextPane(-1);
				break;
			case Note.SCROLL_FAST_SCROLL_UP:
				scrollTextPane(textPane.lineCount / -6);
				break;
			case Note.SCROLL_THIRD_UP:
				scrollTextPane(textPane.lineCount / -3);
				break;
			case Note.SCROLL_PAGE_UP:
				scrollTextPane(-textPane.lineCount);
				break;				
			case Note.SCROLL_LINE_DOWN:
				scrollTextPane(1);
				break;
			case Note.SCROLL_FAST_SCROLL_DOWN:
				scrollTextPane(textPane.lineCount / 6);
				break;				
			case Note.SCROLL_THIRD_DOWN:
				scrollTextPane(textPane.lineCount / 3);				
				break;
			case Note.SCROLL_PAGE_DOWN:
				scrollTextPane(textPane.lineCount);				
				break;				
			default:
				break;
	...
	private function handleResize(event:Event = null):void {
		var appW:int = appStage.stageWidth;
		var appH:int = appStage.stageHeight;
	...
	private function handlePageDown(event:ScrollEvent):void {
		sendNotification(Note.SCROLL_PAGE_DOWN);
	}

	private function handlePageUp(event:ScrollEvent):void {
		sendNotification(Note.SCROLL_PAGE_UP);
	}
